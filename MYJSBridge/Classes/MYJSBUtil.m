...
{
    NSString *interfaceNamespace= @"";

    if (!callString) { *methodName = @""; return interfaceNamespace; }

    *methodName = callString;

    NSRange range = [callString rangeOfString:@"." options:NSBackwardsSearch];

    if(range.location != NSNotFound) {
        interfaceNamespace = [callString substringToIndex:range.location];
        *methodName = [callString substringFromIndex:range.location + 1];
    }
...
{
    if (!object || !sel) return nil;

    NSMethodSignature * sig = [object methodSignatureForSelector:sel];
    if (!sig) { [object doesNotRecognizeSelector:sel]; return nil; }

    NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
    if (!inv) { [object doesNotRecognizeSelector:sel]; return nil; }
    ...
    va_list args;
    va_start(args, sel); 
    [MYJSBUtil setInvocation:inv withSig:sig andArgs:args];
    va_end(args);

    [inv invoke];
    return [MYJSBUtil getReturnFromInv:inv withSig:sig];
}
...
+ (void)setInvocation:(NSInvocation *)inv
              withSig:(NSMethodSignature *)sig
              andArgs:(va_list)args
{
